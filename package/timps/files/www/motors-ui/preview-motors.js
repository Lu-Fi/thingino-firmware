...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Drag-to-point. The joystick is a rate control; this one maps the
  // pointer's offset from where the gesture started straight onto an
  // absolute motor target, so the camera tracks the finger and lands where
  // it is released. Streaming absolute targets is safe by construction:
  // motor_ctl_absolute() re-reads the live position and recomputes its own
  // delta per command, so a superseded target costs nothing.
  function bindDragControls(signal) {
    ...
    function recompute() {
      ...
      if (armed) {
        const lim = limits();
        // screen y grows downward, motor y grows upward
